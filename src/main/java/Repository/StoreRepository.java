package Repository;

import java.io.BufferedReader;
import java.io.BufferedWriter;
import java.io.DataInputStream;
import java.io.FileInputStream;
import java.io.FileWriter;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.ArrayList;

import Model.MyException;
import Model.Product;

public class StoreRepository {
    private ArrayList<Product> allProducts=new ArrayList<Product>();

    public ArrayList<Product> getAllProducts() {
        return allProducts;
    }
    public void readFile(String fname) throws NumberFormatException, IOException{
        FileInputStream f=new FileInputStream(fname);
        DataInputStream in = new DataInputStream(f);
        BufferedReader buf =new BufferedReader(new InputStreamReader(in));
        String rd;
        String []line;
        while((rd=buf.readLine())!=null){
            line=rd.split(" ");
            this.allProducts.add(new Product(Integer.parseInt(line[0]),line[1],line[2],line[3],Integer.parseInt(line[4])));
        }
        in.close();
    }
    @SuppressWarnings("resource")
    public String addNewProduct(Product p) throws IOException, MyException{
        if(p.getCode()>0 && p.getQuantity()>=0 && p.getCode()<Integer.MAX_VALUE&&p.getQuantity()<Integer.MAX_VALUE&& !illegal(p.getName())){
            BufferedWriter out = new BufferedWriter(new FileWriter("products.txt",true));
            int k=1;
            for(Product i:allProducts){
                if(i.getCode()==p.getCode()){
                    k=0;
                }
            }
            if(k==1){
                out.newLine();
                out.write(p.getCode()+" "+p.getName()+" "+p.getCategory()+" "+p.getSupplier() + " "+p.getQuantity());
                out.close();
                allProducts.add(p);
            }
            else{
                out.close();
                throw new MyException("This code already exists");
            }
        }
        else{
            throw new MyException("code and quantity must be positive numbers.");
        }
        return("success");
    }

    private boolean illegal(String name) {
        char c;
        for(int i=0;i<name.length();++i) {
            c = name.charAt(i);
            if (!((c <= 'z' && c >= 'a') || (c <= 'Z' && c >= 'A')|| (c <= '9' && c >= '1')))
                return true;
        }
        return false;
    }

    public ArrayList<Product> getProductsCategory(String cat) throws MyException{
        ArrayList<Product> cProducts=new ArrayList<Product>();
        for(Product p:allProducts){
            if(p.getCategory().compareTo(cat)==0){
                cProducts.add(p);
            }
        }
        if(cProducts.size() == 0) {
            throw new MyException("There are no products for the given category.");
        }
        return cProducts;
    }

    public ArrayList<Product> stockSituationProduct(String pname) throws MyException{
        ArrayList<Product> prods=new ArrayList<Product>();
        for(Product p:allProducts)
            if(p.getName().compareTo(pname)==0)
                prods.add(p);
        if (prods.size() == 0) {
            throw new MyException("There is no such product.");
        }
        return prods;
    }
    public ArrayList<Product> stockSituation() {
        return allProducts;
    }

}
